...
#define ERR_CH579_ERROR			0x81	/* CH579硬件错误,可能需要复位CH579 */
...
#endif
/* 代码2XH-3XH用于USB主机方式的通讯失败代码,由CH579子程序模仿CH375的返回 */
/* 代码1XH用于USB主机方式的操作状态代码,由CH579子程序模仿CH375的返回 */
...
#ifndef MAX_PATH_LEN
#define MAX_PATH_LEN			32		/* 最大路径长度,含所有斜杠分隔符和小数点间隔符以及路径结束符00H */
#endif
...
//	} Open;								/* CMD_FileOpen, 枚举文件, CH579vFileSize最高位为1则各调用xFileNameEnumer,为0则返回指定序号的文件名 */
	struct {
...
/* 子程序库中提供的变量 */
extern	UINT8V	CH579IntStatus;				/* CH579操作的中断状态 */
extern	UINT8V	CH579DiskStatus;			/* 磁盘及文件状态 */
extern	UINT8	CH579vDiskFat;				/* 逻辑盘的FAT标志:1=FAT12,2=FAT16,3=FAT32 */
extern	UINT8	CH579vSecPerClus;			/* 逻辑盘的每簇扇区数 */
extern	UINT8	CH579vSectorSizeB;			/* log2(CH579vSectorSize) */
extern	UINT32	CH579vStartLba;				/* 逻辑盘的起始绝对扇区号LBA */
extern	UINT32	CH579vDiskRoot;				/* 对于FAT16盘为根目录占用扇区数,对于FAT32盘为根目录起始簇号 */
extern	UINT32	CH579vDataStart;			/* 逻辑盘的数据区域的起始LBA */
extern	UINT32	CH579vStartCluster;			/* 当前文件或者目录的起始簇号 */
extern	UINT32	CH579vFileSize;				/* 当前文件的长度 */
extern	UINT32	CH579vCurrentOffset;		/* 当前文件指针,当前读写位置的字节偏移 */
extern	UINT32	CH579vFdtLba;				/* 当前FDT所在的LBA地址 */
extern	UINT32	CH579vLbaCurrent;			/* 当前读写的磁盘起始LBA地址 */
extern	UINT16	CH579vFdtOffset;			/* 当前FDT在扇区内的偏移地址 */
extern	UINT16	CH579vSectorSize;			/* 磁盘的扇区大小 */
extern	UINT8	CH579vCurrentLun;			/* 磁盘当前操作逻辑单元号 */
extern	BOOL	CH579vSubClassIs6;			/* USB存储类设备的子类为6,0则非6 */
extern	PUINT8	pDISK_BASE_BUF;		/* 指向外部RAM的磁盘数据缓冲区,缓冲区长度不小于CH579vSectorSize,由应用程序初始化 */
extern	PUINT8	pDISK_FAT_BUF;		/* 指向外部RAM的磁盘FAT数据缓冲区,缓冲区长度不小于CH579vSectorSize,由应用程序初始化 */
...
#endif
extern	UINT8	CH579ReadSector( UINT8 SectCount, PUINT8 DataBuf );	/* 从磁盘读取多个扇区的数据到缓冲区中 */
#ifdef	EN_DISK_WRITE
extern	UINT8	CH579WriteSector( UINT8 SectCount, PUINT8 DataBuf );	/* 将缓冲区中的多个扇区的数据块写入磁盘 */
#endif

extern	UINT8	CH579DiskConnect( void );	/* 检查磁盘是否连接并更新磁盘状态 */
extern	void	xFileNameEnumer( void );	/* 调用外部定义的子程序,文件名枚举回调子程序 */

extern	UINT8	CH579LibInit( void );		/* 初始化CH579程序库,操作成功返回0 */

/* 子程序库中提供的子程序 */
/* 下述子程序中, 文件操作子程序CH579File*和磁盘查询子程序CH579DiskQuery都可能会用到磁盘数据缓冲区pDISK_BASE_BUF,
   并且有可能在pDISK_BASE_BUF中保存了磁盘信息, 所以必须保证pDISK_BASE_BUF不被用于其它用途,
   如果RAM较少, 要将pDISK_BASE_BUF临时用于其它用途, 那么在临时用完后必须调用CH579DirtyBuffer清除磁盘缓冲区 */
extern	UINT8	CH579GetVer( void );		/* 获取当前子程序库的版本号 */
extern	void	CH579DirtyBuffer( void );	/* 清除磁盘缓冲区 */
extern	UINT8	CH579BulkOnlyCmd( PUINT8 DataBuf );	/* 执行基于BulkOnly协议的命令 */
extern	UINT8	CH579DiskReady( void );		/* 查询磁盘是否准备好 */
extern	UINT8	CH579AnalyzeError( UINT8 iMode );	/* USB操作失败分析CH579IntStatus返回错误状态 */
extern	UINT8	CH579FileOpen( void );		/* 打开文件或者枚举文件 */
extern	UINT8	CH579FileClose( void );		/* 关闭当前文件 */
#ifdef	EN_DISK_WRITE
extern	UINT8	CH579FileErase( void );		/* 删除文件并关闭 */
extern	UINT8	CH579FileCreate( void );	/* 新建文件并打开,如果文件已经存在则先删除后再新建 */
extern	UINT8	CH579FileAlloc( void );		/* 根据文件长度调整为文件分配的磁盘空间 */
#endif
extern	UINT8	CH579FileModify( void );	/* 查询或者修改当前文件的信息 */
extern	UINT8	CH579FileQuery( void );		/* 查询当前文件的信息 */
extern	UINT8	CH579FileLocate( void );	/* 移动当前文件指针 */
extern	UINT8	CH579FileRead( void );		/* 从当前文件读取数据到指定缓冲区 */
#ifdef	EN_DISK_WRITE
extern	UINT8	CH579FileWrite( void );		/* 向当前文件写入指定缓冲区的数据 */
#endif
extern	UINT8	CH579ByteLocate( void );	/* 以字节为单位移动当前文件指针 */
extern	UINT8	CH579ByteRead( void );		/* 以字节为单位从当前位置读取数据块 */
#ifdef	EN_DISK_WRITE
extern	UINT8	CH579ByteWrite( void );		/* 以字节为单位向当前位置写入数据块 */
#endif
extern	UINT8	CH579DiskQuery( void );		/* 查询磁盘信息 */
extern	void	CH579SaveVariable( void );	/* 备份/保存/恢复子程序库的变量,用于子程序库在多个芯片或者U盘之间进行切换 */

extern	void	mDelayuS( UINT16 n );		// 以uS为单位延时
extern	void	mDelaymS( UINT16 n );		// 以mS为单位延时
extern	UINT8	USBHostTransact( UINT8 endp_pid, UINT8 tog, UINT32 timeout );	// CH579传输事务,输入目的端点地址/PID令牌,同步标志,NAK重试时间,返回0成功,超时/出错重试
